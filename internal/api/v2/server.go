...
func (s *Server) Register(mux *http.ServeMux) {
	...
	mux.HandleFunc("GET /api/v2/calls", s.auth(s.handleListCalls))
	mux.HandleFunc("POST /api/v2/calls/clear", s.auth(s.handleClearCalls))
	mux.HandleFunc("GET /api/v2/calls/{id...}", s.auth(s.handleGetCall))
	...
	mux.HandleFunc("GET /api/v2/jobs/{id}/artifacts/{kind}", s.auth(s.handleDownloadJobArtifact))
	mux.HandleFunc("GET /api/v2/reports", s.auth(s.handleListReports))
