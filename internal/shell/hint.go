...
func printSetCheatsheet(out io.Writer) {
	...
	fmt.Fprintln(out, "  set builtin_scenario uac|uas|invite_media|invite_media_scale   — built-in scenario (short: sn)")
	...
	fmt.Fprintln(out, "Flag help: gossipper sipp -h (full SIPp-compat); gossipper server -h (management groups). Smarter review:  hint")
}
