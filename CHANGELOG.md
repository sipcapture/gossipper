...
- Live Trace: one process-wide circular buffer (`siplog.Ring`, cap 1024) for SIP (UAS listen UDP and in-process scenario engines) plus scenario/app debug during a call. Control UI **Live Trace** (`#/sip`) has independent **SIP** and **Debug** capture buttons plus an on-the-fly Debug level (`debug`/`info`/`warn`/`error`; default `debug`). `GET/PUT /api/v2/sip/trace/capture` is `{ sip, app, level }` (SIP default on, debug default off; empty PUT `level` keeps the previous floor). WebSocket `GET /api/v2/sip/trace/ws` (`?token=` for JWT). REST `GET /api/v2/sip/trace?since=` remains for scripts. **Text** / **PCAP** export the whole ring (`GET /api/v2/sip/trace/export?format=text|pcap`; pcap is SIP only). Originate UAC jobs still run out of process and are not in this log.
